...
template<class Key, class Value> class bstree {
  ...
  public:
   ...
   class iterator_inorder {  // Q: Can this class handle also handle reverse iterator logic? Reverse iterators can move forward and backward. See ~/t4 iterator code.
   
      using node_type = bstree<Key, Value>::node_type;
   
      node_type *current;
      /*
         beg -- iterator at beginning of container
         between -- in between beg and end positions
         end -- iterator at one-past-end of container
       */
     
      enum class position {beg, between, end};
      position pos;
     ...
     public:
      ...
      explicit iterator_inorder(bstree<Key, Value>& bstree) : tree{bstree}, pos{beg}
      {
         pos = end;  
         current = (size == 0) ? nullptr : current = max(bstree.root.get());
      }
   
      // Ctor for reverse iterators 
      iterator_inorder(bstree<Key, Value>& bstree, int dummy) : tree{bstree}, pos{end}
      {
         pos = beg;

         // Set current to node with largest key, if tree not empty.
         current = (size == 0) ? nullptr : current = max(bstree.root.get());
      }
      ...
      iterator_inorder(iterator_inorder&& lhs) : current{lhs.current}, tree{lhs.tree}, pos{lhs.pos}
      {
          lhs.current = nullptr;
          lhs.size = 0;

          // What if it is a reverse_iterator--I think we are o.k.
      }
