  ...
  public:
   ...
   class iterator_inorder {  
     ...
     public:
      ...
      iterator_inorder() : current{nullptr}, ptree{nullptr}, pos{??}
      {
      }

      explicit iterator_inorder(bstree<Key, Value>& tree) : ptree{&tree}, pos{???}
      {
      ...
      /*
      iterator_inorder(bsptree<Key, Value>& bsptree, int dummy) : ptree{bsptree}, pos{???}
      {
         // Set current to nodee with smallest key.
